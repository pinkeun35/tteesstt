...
function list_reload(vPage) {
	var bbs_name = "bbs2";
	$('#now_page').val(vPage);
	$.post( "/community/get_bbs_mylist", { bbs: bbs_name, page: vPage, spart: $('#spart').val(), skeyword: $('#skeyword').val() })
		.done(function( jdata ) {
			if (jdata.status == "success") {
				...
				if ( jdata.item.length > 0 ) {
					...
					$.each(jdata.item, function(key,state){
						...
						tr_html += '<td' + reply_step + '>' + reply_step_image + '<a href="javascript:get_record(\'' + bbs_name + '\',' + jdata.item[key].seq + ');" class="btn_diapup158">' + jdata.item[key].title + '</a></td>';
...
function get_record(vBbs, vSeq) {
	$('#bbs').val(vBbs);
	$.post( '/community/get_bbs_record', { bbs: vBbs, record: vSeq })
		.done(function( data ) {
...
<div id="containerwrap">
	<div id="container">
		...
		<h2 class="titlebox"><span class="title_1_7"><img src="/jsp/images/common/img_subtitle1_7.gif" alt="MEMBER" /></span></h2>            

        <div class="path"><p><a href="#;"><img src="/jsp/images/common/icon_loc.gif" alt="HOME" /></a> <span>&gt;</span> <a href="#;">MEMBER</a> <span>&gt;</span> <strong>내게시물</strong></p></div>

        <!-- 본문 -->
        <div class="contentswrap">
        	<p><strong><?php echo $session_id; ?></strong>님이 자유게시판 에작성한 게시물입니다.</p>
        	...
        	<p><strong><?php echo $session_id; ?></strong>님이 자료실에 작성한 게시물입니다.</p>
            ...
            <dl class="tablebox1">
                ...
                <dd class="bbsbtnbox1">
                    <!--div class="btntype1"><button type="button">목록</button></div-->
                    <div class="btntype2"><?php if ($bbs_write && $session_id != "") { ?><button type="button" class="st1" onclick="go_write('bbs3');">작성</button><? } ?></div>
