...
// 재계산
function againCalcul(vRecord){
	var action_url = '/sdata/setAgainCalcul';
	$.post( action_url, {
		record : vRecord,
		vol : $('#jarea_03').text(),
		weight : $('#jarea_05').text(),
		pre_use : $('#jarea_15').text(),
		pre_use_sb : $('#jarea_16').text(),
		pre_use_cfc : $('#jarea_17').text(),
		pre_use_so : $('#jarea_18').text(),
		pre_use_po : $('#jarea_19').text(),
		pre_use_hch : $('#jarea_20').text(),
		pre_use_ho : $('#jarea_21').text()
	}).done(function( jdata ) {
		if (jdata.status == "success") {
			alert("재계산 하였습니다.");
			$( "#diapup137" ).dialog( "close" );
...
function init_pop_infojajae(jpreuse, jinfo, joccurrence) {
	...
	// 자재사용정보
	$('#jarea_01').html(jpreuse.jname); // 자재명
	$('#jarea_02').html(jpreuse.jstandard); // 자재규격
	$('#jarea_03').html(jpreuse.jvolume); // 수량
	$('#jarea_04').html(jpreuse.junit); // 단위
	$('#jarea_05').html(jpreuse.weight); // 무게

	// 자재기준정보
	$('#jarea_06').html(jinfo.jname); // DB 명칭
	$('#jarea_07').html(jinfo.jstandard); // DB 규격
	$('#jarea_08').html(""); // 배출기준
	$('#jarea_09').html(jinfo.junit); // 기준 단위
	$('#jarea_10').html(jinfo.jweight); // 무게기준
	$('#jarea_11').html(jinfo.jjugi); // 수선주기
	$('#jarea_12').html(jinfo.jsuseon); // 수선율
	if (jinfo.jrecycle == "0") 
		$('#jarea_13').html("폐기"); // 폐기재활용
	else if (jinfo.jrecycle == "1")
		$('#jarea_13').html("재활용"); // 폐기재활용
	$('#jarea_14').html(jinfo.jculceo); // 출처
	$('#jarea_15').html(jinfo.jco2); // 지구온난화
	$('#jarea_16').html(jinfo.jsb); // 자원소모
	$('#jarea_17').html(jinfo.jcfc); // 오존층영향
	$('#jarea_18').html(jinfo.jso); // 산성화
	$('#jarea_19').html(jinfo.jpo); // 부영양화
	$('#jarea_20').html(jinfo.jhch); // 광화확적산화물
	$('#jarea_21').html(jinfo.jho); // 물발자국
	...
	if ( joccurrence.length > 0 ) {
		$.each(joccurrence, function(key,state){
			if ((key + 1) < joccurrence.length) {
				...
				tr_html += '<td class="c">' + joccurrence[key].pre_use_sb + '</td>';
				tr_html += '<td class="c">' + joccurrence[key].pre_use_cfc + '</td>';
				tr_html += '<td class="c">' + joccurrence[key].pre_use_so + '</td>';
				tr_html += '<td class="c">' + joccurrence[key].pre_use_po + '</td>';
				tr_html += '<td class="c">' + joccurrence[key].pre_use_hch + '</td>';
				tr_html += '<td class="c">' + joccurrence[key].pre_use_ho + '</td>';
				tr_html += '</tr>';
			}
		});
	} else {
...
<div class="contentswrap">
    <dl class="tablebox1">
        <dd>
            ...
            <table id="info_table_j2" class="tablelisttype1" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr class="mobg">
                	...
                	<td class="c"><span id="jarea_13"></span></td>
                	<td class="c"><span id="jarea_14"></span></td>
                </tr>
           <!-- </table>
            <table id="info_table_j3" class="tablelisttype1" border="0" cellpadding="0" cellspacing="0">-->
                <tr>
                    <th class="tl">지구온난화</th>
                    <th>자원소모</th>
                    <th>오존층영향</th>
                    <th>산성화</th>
                    <th>부영양화</th>
                ...
                <tr class="mobg">
               		<td class="c"><span id="jarea_15"></span></td>
                	<td class="c"><span id="jarea_16"></span></td>
                	<td class="c"><span id="jarea_17"></span></td>
                	<td class="c"><span id="jarea_18"></span></td>
                	<td class="c"><span id="jarea_19"></span></td>
                	<td class="c"><span id="jarea_20"></span></td>
                	<td class="c"><span id="jarea_21"></span></td>
                	<!--<td class="c"><span id="jarea_22"></span></td>
                	<td class="c"><span id="jarea_23"></span></td>-->
                </tr>
            </table>
            <table id="info_table_m2" class="tablelisttype1" border="0" cellpadding="0" cellspacing="0">
                ...
                <tr class="mobg">
                	...
                	<td class="c"><span id="marea_10"></span></td>
                	<td class="c"><span id="marea_11"></span></td>
                </tr>
            <!--</table>
            <table id="info_table_m3" class="tablelisttype1" border="0" cellpadding="0" cellspacing="0">-->
                <tr>
                    <th class="tl">지구온난화</th>
                    <th>자원소모</th>
                    <th>오존층영향</th>
                ...
                <tr class="mobg">
               		<td class="c"><span id="marea_12"></span></td>
                	<td class="c"><span id="marea_13"></span></td>
                	<td class="c"><span id="marea_14"></span></td>
                	<td class="c"><span id="marea_15"></span></td>
                	<td class="c"><span id="marea_16"></span></td>
                	<td class="c"><span id="marea_17"></span></td>
                	<td class="c"><span id="marea_18"></span></td>
                	<!--<td class="c"><span id="marea_19"></span></td>
                	<td class="c"><span id="marea_20"></span></td>-->
                </tr>
            </table>
        </dd>
        <dd>
            <p class="bbstitle1">환경영향범주</p>
            ...
            <table id="area_table1" class="tablelisttype1" border="0" cellpadding="0" cellspacing="0">
                <tr>
                    <th class="tl">지구온난화<br>(Kgco2)</th>
                    <th class="tl">자원소모<br>(Kgsb)</th>
                    <th class="tl">오존층영향<br>(Kgcfc11)</th>
                    <th class="tl">산성화<br>(Kgso2)</th>
                    <th class="tl">부영양화<br>(Kgpo4 3-)</th>
                    <th class="tl">광화학적산화물<br>(Kgc2h4)</th>
                    <th class="tl">물발자국<br>(Kgh2o)</th>
                </tr>
                <tr>
                	<td class="c">1</td>
                	<td class="c">1000</td>
        ...
        <dd class="bbsbtnbox1">
            <div class="btntype3"><button type="button" id="cbtn_diapup127">닫기</button> <button type="button" id="mbtn_diapup127" class="st3" onclick="get_record_modify($('#pjseq').val());">수정</button> <button type="button" class="st1" id="rbtn_diapup127" onclick="againCalcul($('#pjseq').val());">재계산</button> <button type="button" class="st5" id="dbtn_diapup127">삭제</button></div>
        </dd>
    </dl>
</div>
</div>
