			...
			if (jdata.status == "success") {
				...
				if ( jdata.item.length > 0 ) {
					...
					//	dd_html += '<th colspan="4">1공구<button  class="mbtn_diapup111">공구수정</button> <button class="btn_diapup115">건물설정</button></th>';
						dd_html += '<th colspan="5">' + jdata.item[key].zone_name + '<button class="" onclick="get_gonggu_modify(' + jdata.item[key].seq + ');">공구수정</button> <button class="" onclick="pop_building_insert(<?php echo $param['seq'];?>,' + jdata.item[key].seq + ');">건물추가</button></th>';
						dd_html += '</tr>';
