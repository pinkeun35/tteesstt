...
class Admin_model extends CI_Model {
	...
	function update_lcijajae($arrays) {
		...
		$sql = "UPDATE lci_jajae SET jname=?, jstandard=?, jco2=?, junit=?, jjugi=?, jsuseon=?, jweight=?, jrecycle=?, jculceo=?, jupdate=?, edate=NOW() WHERE seq = ?";
	...
	function update_lcienergy($arrays) {
		...
		$sql = "UPDATE lci_energy SET ename=?, ecaloric=?, eco2=?, eunit=?, eculceo=?, eupdate=?, edate=NOW() WHERE seq = ?";
		$query = $this->db->query($sql, $update_array);
	...
	function update_lcimachine($arrays) {
		...
		$sql = "UPDATE lci_machine SET mname=?, mstandard=?, mileage=?, munit=?, mco2=?, mculceo=?, mupdate=?, edate=NOW() WHERE seq = ?";
		$query = $this->db->query($sql, $update_array);
	...
	function get_lcijajae_list($num_start, $num_record) {
		$sql = "SELECT seq, jname, jstandard, jco2, junit, jjugi, jsuseon, jweight, jrecycle, jculceo, jupdate, uid, wdate, edate FROM lci_jajae ORDER BY seq DESC LIMIT ?,?";
	...
	function get_lcijajae_keywordlist($keyword, $num_start, $num_record) {
		$sql = "SELECT seq, jname, jstandard, jco2, junit, jjugi, jsuseon, jweight, jrecycle, jculceo, jupdate, uid, wdate, edate FROM lci_jajae WHERE jname LIKE ? ORDER BY seq DESC LIMIT ?,?";
	...
	function get_lcienergy_list($num_start, $num_record) {
		$sql = "SELECT seq, ename, ecaloric, eco2, eunit, eculceo, eupdate, uid, wdate, edate FROM lci_energy ORDER BY seq DESC LIMIT ?,?";
		$query = $this->db->query($sql, array($num_start, $num_record));
		$rows = $query->result();
	...
	function get_lcienergy_keywordlist($keyword, $num_start, $num_record) {
		$sql = "SELECT seq, ename, ecaloric, eco2, eunit, eculceo, eupdate, uid, wdate, edate FROM lci_energy WHERE ename LIKE ? ORDER BY seq DESC LIMIT ?,?";
		$query = $this->db->query($sql, array('%'.$keyword.'%', $num_start, $num_record));
		$rows = $query->result();
	...
	function get_lcimachine_list($num_start, $num_record) {
		$sql = "SELECT seq, mname, mstandard, mileage, munit, mco2, mculceo, mupdate, uid, wdate, edate FROM lci_machine ORDER BY seq DESC LIMIT ?,?";
		$query = $this->db->query($sql, array($num_start, $num_record));
		$rows = $query->result();
	...
	function get_lcimachine_keywordlist($keyword, $num_start, $num_record) {
		$sql = "SELECT seq, mname, mstandard, mileage, munit, mco2, mculceo, mupdate, uid, wdate, edate FROM lci_machine WHERE mname LIKE ? ORDER BY seq DESC LIMIT ?,?";
		$query = $this->db->query($sql, array('%'.$keyword.'%', $num_start, $num_record));
		$rows = $query->result();
	...
	function get_lcijajae_record($record) {
		$sql = "SELECT seq, jname, jstandard, jco2, junit, jjugi, jsuseon, jweight, jrecycle, jculceo, jupdate, uid, wdate, edate FROM lci_jajae WHERE seq = ?";
	...
	function get_lcienergy_record($record) {
		$sql = "SELECT seq, ename, ecaloric, eco2, eunit, eculceo, eupdate, uid, wdate, edate FROM lci_energy WHERE seq = ?";
		$query = $this->db->query($sql, $record);
		$row = $query->row(0);
	...
	function get_lcimachine_record($record) {
		$sql = "SELECT seq, mname, mstandard, mileage, munit, mco2, mculceo, mupdate, uid, wdate, edate FROM lci_machine WHERE seq = ?";
		$query = $this->db->query($sql, $record);
		$row = $query->row(0);
	...
	function get_lcijajae_search($s_text) {
		$sql = "SELECT seq, jname AS vname, jstandard AS vstandard, jco2 AS vco2, junit AS vunit, jweight AS vweight FROM lci_jajae WHERE jname LIKE ? ORDER BY seq ASC";
	...
	function get_lcimachine_search($s_text) {
		$sql = "SELECT seq, mname AS vname, mstandard AS vstandard, mco2 AS vco2, munit AS vunit, mileage AS vweight FROM lci_machine WHERE mname LIKE ? ORDER BY seq ASC";
